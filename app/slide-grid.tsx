...
export default function SlideGrid({slides,selected,onClose,onSelect,thumbnail}:{slides:Slide[];selected:number;onClose:()=>void;onSelect:(index:number)=>void;thumbnail:(slide:Slide,index:number)=>ReactNode}){
 ...
 return <dialog ref={dialog} className="slide-grid-dialog" aria-modal="true" aria-labelledby="slide-grid-title" onCancel={e=>{e.preventDefault();onClose();}} onKeyDown={e=>{if(!((e.metaKey||e.ctrlKey)&&['c','v'].includes(e.key.toLowerCase())))e.stopPropagation();}}>
  ...
  <div className="slide-grid-list" style={{'--grid-thumbnail-size':`${size}px`} as CSSProperties}>
   {slides.map((slide,index)=><button type="button" className="slide-grid-card" data-slide-id={slide.id} key={slide.id} aria-pressed={selected===index} aria-current={selected===index?'true':undefined} aria-label={`スライド ${index+1} を選択: ${slide.title.replaceAll('\n',' ')}`} onClick={()=>onSelect(index)}>{thumbnail(slide,index)}<span className="slide-grid-caption"><b>{index+1}</b><span>{slide.title.replaceAll('\n',' ')||'無題のスライド'}</span></span></button>)}
  </div>
 </dialog>;
}
