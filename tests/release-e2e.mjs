...
console.log('PASS: three public backgrounds selected/rendered/persisted');
...
await title.fill('通信失敗から復旧');await title.press('Tab');
...
await title.fill('保存してからプレゼン');await title.press('Tab');
...
await title.fill('新しい版を保持');await title.press('Tab');
await p.waitForFunction(()=>document.querySelector('.projectbar')?.textContent?.includes('保存済み'));
