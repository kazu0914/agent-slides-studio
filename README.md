...
- プレゼン中のメモ自動保存・スライド別マーカー保持、ズーム、クリック表示、別ウィンドウの発表者画面
...
AI入力欄の「Codex標準のWeb検索を使う」をオンにすると、必要に応じてWebを調べて提案します（初期状態はオフ）。調査時は出典URLと確認日を回答・スピーカーノートに残すよう指示しています。出典と内容は利用前に確認してください。現在日時とタイムゾーンは自動で渡します。Codexの利用条件・利用枠が適用され、すべてのCodexツールを許可する機能ではありません。社内資料や個人情報を調査に使う際は共有範囲に注意してください。

選択ツールでスライドの余白をドラッグすると、カード・既存の文字・自由配置要素を範囲選択できます。選択した要素をドラッグしてまとめて移動し、Shiftで追加選択、余白クリック／Escで解除します。ロック・非表示の要素は範囲選択から除外します。移動はマウスを離したときにまとめて記録され、一度のUndoで戻せます。

カードは選択後の上部ツールバーの「カードをすべて選択」、各カードの「選択」、Shift＋クリックで複数選択できます。選択したカードの「移動」をドラッグするか、フォーカスして矢印キーを押すとまとめて移動できます。自由配置要素はShift／⌘＋クリックで複数選択してドラッグできます。
...
プレゼンの操作バーから「メモ」を開くと、現在のスライドにメモを残せます。通常の編集画面では「プレゼン中のメモ」から確認・修正できます。スピーカーノートとは別です。投影画面にメモを見せたくない場合は、別ウィンドウの「発表者画面」で入力してください（画面共有時は共有対象にも注意してください）。

マーカーはスライドを移動しても保持され、プレゼンを終了すると編集画面では非表示になります。次回のプレゼンで再表示され、「消去」は現在のスライドのマーカーだけを消します。

メモとマーカーは、このブラウザの同じURLの保存領域に保存されます。ブラウザやURL（localhost / 127.0.0.1）を変えると共有されません。ブラウザデータの削除で消えるため、重要なメモは別途コピーしてください。現在、スライドのバックアップ・PDF・PowerPointには含まれません。

スライドを複製するには、左の一覧またはグリッドでスライドを選択して⌘C、貼り付け先のスライドを選んで⌘Vを押します（WindowsはCtrl）。選択したスライドの直後にコピーが追加されます。テキストボックスなどの要素は選択してDelete／Backspaceで削除でき、Undoで戻せます。文字を編集中は通常どおり文字を削除し、ロックされた要素は削除しません。

複数の要素を選択すると、上部に「整列・等間隔」が表示されます。左右・上下の端や中央を揃えられ、3つ以上なら横・縦の間隔を均等にできます。選択範囲を基準に配置し、Undoで戻せます。

選択ツールで要素を動かすと、揃う位置にピンクの点線と座標、隣接する要素との余白が表示されます（pxはスライドの1600×900座標）。近い位置に吸着し、Option／Altを押しながら動かすと吸着を一時解除できます。
